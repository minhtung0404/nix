{ config, pkgs, lib, ... }:
let cfg = config.mtn.services.my-aerospace;
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.aerospace = {
      ...
      userSettings = {
        ...
        in builtins.listToAttrs (map (f "cmd-" "workspace ") (range 9))
        // builtins.listToAttrs
        (map (f "cmd-shift-" "move-node-to-workspace --focus-follows-window ")
          (range 9)));
