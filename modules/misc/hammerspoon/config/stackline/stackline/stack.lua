...
function Stack:getWindowByPoint(p)
  if p.x < 0 or p.y < 0 then
    ...
    local function findClickedScreen(_p) -- {{{
      return table.unpack(u.filter(hs.screen.allScreens(), function(s)
        return _p:inside(s:frame())
      end))
    end -- }}}

    local clickedScren = findClickedScreen(p)
    p = clickedScren and clickedScren:absoluteToLocal(p) or p
  end

  return table.unpack(u.filter(self.windows, function(w)
