...
let
  ...
  languageServerOption = types.submodule {
    options = {
      ...
      roots = mkOption {
        type = types.listOf types.str;
        description =
          "The list of root filenames that are used to determine the project root";
      };
      command = mkOption {
        type = types.str;
      ...
      package = mkOption {
        ...
        default = null;
        description =
          "The default package of the language server. Will be appended as the ending segments of the PATH to kak-lsp";
      };
    };
  };

  cfg = config.mtn.programs.kak-lsp;

  serverPackages = filter (v: v != null)
  ...
  wrappedPackage = pkgs.symlinkJoin {
    ...
    paths = [ cfg.package ];
    postBuild = ''
      wrapProgram $out/bin/kak-lsp --suffix PATH ":" ${
        lib.makeBinPath serverPackages
      }
    '';
  };
in {
