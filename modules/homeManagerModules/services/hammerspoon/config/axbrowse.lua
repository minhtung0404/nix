...
local buildChoicesForObject = function(obj)
  ...
  for k, v in fnutils.sortByKeys(aav) do
    ...
    if objIsTable then
      ...
      entry.cmdAddition = "." .. k
      if entry.settable then
        entry.altCmd = entry.cmdAddition .. " = ..."
      end
    end
...
local chooserCallback = function(item)
  ...
  if item.text:match("^<--") then
    ...
    if objDetails.attribute then
      obj = obj[objDetails.attribute]
    end
  ...
  if item.attribute then
    ...
    if type(obj) == "table" then
      objDetails.tableAttribute = item.attribute
    end
  end
  ...
  if not item.cmdNoAdd then
    storage._path = storage._path .. (item.cmdAddition or "")
  end
